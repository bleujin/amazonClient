...
public class S3FileObject extends AbstractFileObject {
	...
	final private S3Service service;

	final private S3Bucket bucket;
	private StorageObject object;
	private boolean attached = false;

	private boolean downloaded = false; //True when content downloaded. It's an extended flag to <code>attached</code>.

	private File cacheFile; // * Local cache of file content
	private StorageOwner fileOwner; //Amazon file owner. Used in ACL
	private Log logger = LogFactory.getLog(S3FileObject.class);

	public S3FileObject(AbstractFileName fileName, S3FileSystem fileSystem, S3Service service, S3Bucket bucket) throws FileSystemException {
	...
	protected void doCreateFolder() throws Exception {
		if (logger.isDebugEnabled()) {
			logger.debug("Create new folder in bucket [" + ((bucket != null) ? bucket.getName() : "null") + "] with key [" + ((object != null) ? object.getKey() : "null") + "]");
		}
	...
	protected String[] doListChildren() throws Exception {
		...
		for (int i = 0; i < children.length; i++) {
			if (!children[i].getKey().equals(path)) {
				...
				if ((stripPath.endsWith(SEPARATOR) && (stripPath.indexOf(SEPARATOR_CHAR) == stripPath.lastIndexOf(SEPARATOR_CHAR))) || (stripPath.indexOf(SEPARATOR_CHAR) == (-1))) {
	...
	/**
	 * Download S3 object content and save it in temporary file. Do it only if object was not already downloaded.
	 */
	...
	/**
	 * Create an S3 key from a commons-vfs path. This simply strips the slash from the beginning if it exists.
	 * 
	 ...
	 * 
	 * VFS interfaces doesn't provide interface to manage permissions. ACL can be accessed through {@link FileObject#getFileOperations()} Sample: <code>file.getFileOperations().getOperation(IAclGetter.class)</code>
	 * 
	...
	public String getPrivateUrl() {
		return String.format("s3://%s:%s@%s/%s", service.getProviderCredentials().getAccessKey(), service.getProviderCredentials().getSecretKey(), bucket.getName(), getS3Key());
	}
	...
	/**
	 * Special JetS3FileObject output stream. It saves all contents in temporary file, onClose sends contents to S3.
	 * 
